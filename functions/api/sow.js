  ...
  try {
    ...
    return secureJson({ sow: {
      ...
      paymentUrl: sow.status === "approved" && sow.billing_status !== "active" && sow.payment_status !== "paid"
        ? `${env.PUBLIC_SITE_URL}/payment.html?id=${sow.id}&token=${encodeURIComponent(token)}`
        : null,
  ...
  try {
    ...
    return secureJson({
      ...
      paymentUrl: action === "approved"
        ? `${env.PUBLIC_SITE_URL}/payment.html?id=${sow.id}&token=${encodeURIComponent(data.token)}`
        : null,
    });
  } catch (error) {
    return handleError(error);
