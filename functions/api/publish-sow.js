...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const reviewUrl = `${env.PUBLIC_SITE_URL}/scope/?id=${id}&token=${reviewToken}`;

    await sendOutlookMail(
      env,
      clientEmail,
      `${customerNumber} · Scope ready for review: ${title}`,
      `<p>Hello ${escapeHtml(clientName)},</p><p>Your project scope and quote for <strong>${escapeHtml(customerNumber)}</strong> are ready.</p><p><strong>${escapeHtml(paymentLabel)}:</strong> $${(amountCents / 100).toFixed(2)}${billingType === "recurring_monthly" ? " per month with automatic monthly billing" : ""}</p><p><a href="${escapeHtml(reviewUrl)}">Review and approve the SOW</a></p><p>After approval, the same secure link will take you directly to payment setup. It expires in 30 days.</p>`,
    );
