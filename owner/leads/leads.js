(() => {
  ...
  async function load() {
    ...
    const body = await api(`/api/leads?${params}`);
  ...
  $("#lead-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const data = Object.fromEntries(form.entries()); data.fitReasons = data.fitReasons ? data.fitReasons.split(",") : []; try { await api("/api/leads", { method: "POST", body: JSON.stringify(data) }); $("#lead-dialog").close(); await load(); } catch (error) { if (error.status === 409 && error.body?.duplicates?.length && window.confirm(`${error.message}\n\n${error.body.duplicates.map((lead) => lead.business_name).join(", ")}\n\nSave this as a separate record after review?`)) { try { data.confirmDuplicate = true; await api("/api/leads", { method: "POST", body: JSON.stringify(data) }); $("#lead-dialog").close(); await load(); return; } catch (retryError) { error = retryError; } } $("#lead-form-error").textContent = error.message; } });
